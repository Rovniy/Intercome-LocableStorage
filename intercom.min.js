...
var Intercom = function () {
    ...
    g.createInterface = function (b) {
        return {
            on: function (a, c) {
                ...
                if ("undefined" !== typeof this[b] && this[b].hasOwnProperty(a)) for (var c = Array.prototype.slice.call(arguments, 1), e = 0; e < this[b][a].length; e++) this[b][a][e].apply(this[b][a][e], c)
            }
        }
    };
    ...
    i.extend = function (b, a) {
        ...
        if ("object" === typeof a) for (var c in a) a.hasOwnProperty(c) && (b[c] = a[c]);
        return b
    };
    ...
    d.prototype._transaction = function (b) {
        var a = this, c = !1, e = !1, p = null, d = function () {
            if (!c) {
                var g = (new Date).getTime(), s = parseInt(f.getItem(l) || 0);
                s && 1E3 > g - s ? (e || (a._on("storage", d), e = !0), p = window.setTimeout(d, 20)) : (c = !0, f.setItem(l, g), b(), e && a._off("storage", d),
                p && window.clearTimeout(p), f.removeItem(l))
            }
        };
        d()
    };
    d.prototype._cleanup_emit = i.throttle(100, function () {
        this._transaction(function () {
            for (var b = (new Date).getTime() - t, a = 0, c = JSON.parse(f.getItem(j) || "[]"), e = c.length - 1; 0 <= e; e--) c[e].timestamp < b && (c.splice(e, 1), a++);
    ...
    d.prototype._cleanup_once = i.throttle(100, function () {
        ...
        this._transaction(function () {
            ...
            (new Date).getTime();
            var e = 0;
            for (a in c) b._once_expired(a, c) && (delete c[a],
                e++);
    ...
    d.prototype._once_expired = function (b, a) {
        ...
        var c = a[b].ttl || u, e = (new Date).getTime();
    ...
    d.prototype._onStorageEvent = function (b) {
        var b = b || window.event, a = this;
        this._localStorageChanged(b, j) && this._transaction(function () {
            for (var b =
                (new Date).getTime(), e = f.getItem(j), e = JSON.parse(e || "[]"), d = 0; d < e.length; d++) if (e[d].origin !== a.origin && !(e[d].timestamp < a.lastMessage)) {
                if (e[d].id) {
    ...
    d.prototype._emit = function (b, a, c) {
        ...
        var e = {
            id: c, name: b, origin: this.origin,
            timestamp: (new Date).getTime(), payload: a
        }, d = this;
        this._transaction(function () {
            var c = f.getItem(j) || "[]",
        ...
        function (b, a, c) {
            if (d.supported) {
                ...
                this._transaction(function () {
                    ...
                    e._once_expired(b, d) && (d[b] = {}, d[b].timestamp = (new Date).getTime(), "number" === typeof c && (d[b].ttl = 1E3 * c), f.setItem(k, JSON.stringify(d)), a(), window.setTimeout(function () {
                        e._cleanup_once()
                    }, 50))
                })
            }
    ...
    var j = "intercom", k = "intercom_once", l = "intercom_lock", t = 5E4, u = 36E5;
    d.destroy = function () {
        f.removeItem(l);
        f.removeItem(j);
        f.removeItem(k)
    ...
    var r = function (b, a, c) {
        ...
        if (a.receive) {
            ...
            for (g in c.handlers) for (var h = 0; h < c.handlers[g].length; h++) f(g, c.handlers[g][h]);
            c._on("on", f)
        }
        a.send && c._on("emit", function (c, d) {
            var e =
                "function" === typeof a.send ? a.send(c, d) : !0;
